...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="mb-12">
            ...
            <div class="flex flex-wrap gap-1.5 sm:gap-2.5 items-center">
                @php
                    $isActiveAll = !request('category');
                @endphp
                <a href="/"
                    class="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-6 sm:py-3 font-semibold text-xs sm:text-sm tracking-wide rounded-xl sm:rounded-2xl transition-all duration-300 transform whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-offset-2 leading-none
                    {{ $isActiveAll
                    ...
                    <i class="fa-solid fa-layer-group text-[10px] sm:text-sm"></i>
                    <span>Semua</span>
                    <span class="px-1.5 py-0.5 text-[9px] sm:text-[10px] font-black rounded-full {{ $isActiveAll ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500' }}">{{ \App\Models\Event::where('date', '>=', now())->count() }}</span>
                </a>
                @foreach($categories as $cat)
                    ...
                    <a href="?category={{ $cat->slug }}"
                        ...
                        @if(strtolower($cat->name) === 'seminar')
                            <i class="fa-solid fa-chalkboard-user text-[10px] sm:text-sm"></i>
                        @elseif(str_contains(strtolower($cat->name), 'enterta') || str_contains(strtolower($cat->name), 'hiburan') || str_contains(strtolower($cat->name), 'concert'))
                            <i class="fa-solid fa-music text-[10px] sm:text-sm"></i>
                        @elseif(strtolower($cat->name) === 'workshop')
                            <i class="fa-solid fa-wrench text-[10px] sm:text-sm"></i>
                        @else
                            <i class="fa-solid fa-tag text-[10px] sm:text-sm"></i>
                        @endif
                        <span>{{ $cat->name }}</span>
                        <span class="px-1.5 py-0.5 text-[9px] sm:text-[10px] font-black rounded-full {{ $isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500' }}">{{ $catCount }}</span>
                    </a>
                @endforeach
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
            @foreach($events as $event)
                <div class="group bg-white rounded-xl sm:rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    <div class="relative overflow-hidden aspect-[3/4]">
                        ...
                        <div class="absolute top-2 left-2 sm:top-4 sm:left-4 px-1.5 py-0.5 sm:px-3 sm:py-1 bg-white/90 backdrop-blur rounded-md sm:rounded-lg text-[9px] sm:text-xs font-bold uppercase text-indigo-600">
                            {{ $event->category->name }}
                        </div>
                    </div>

                    <div class="p-2.5 sm:p-6">
                        <h3 class="text-xs sm:text-xl font-bold mb-1 sm:mb-2 line-clamp-2 sm:line-clamp-1 group-hover:text-indigo-600 transition">
                            {{ $event->title }}
                        </h3>

                        <div class="flex items-center gap-1.5 sm:gap-2 text-slate-500 text-[10px] sm:text-sm mb-2 sm:mb-4">
                            <i class="fa-solid fa-clock w-3 sm:w-4 text-center"></i>
                            <span class="truncate">{{ $event->date }}</span>
                        </div>

                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 pt-2 sm:pt-4 border-t">
                            <span class="text-sm sm:text-2xl font-black text-indigo-600">
                                Rp {{ number_format($event->price, 0, ',', '.') }}
                            </span>

                            <a href="{{ route('events.show', $event->id) }}"
                                class="px-2.5 py-1.5 sm:px-5 sm:py-2 bg-indigo-50 text-indigo-600 rounded-lg sm:rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition flex items-center justify-center gap-1.5 sm:gap-2 text-[11px] sm:text-base">
                                <i class="fa-solid fa-eye w-3 sm:w-4"></i>
    ...
    <section class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="relative" x-data="{ scrollContainer: null }" x-init="scrollContainer = $refs.carousel">
            ...
            <div x-ref="carousel" class="flex gap-3 sm:gap-6 overflow-x-auto scroll-smooth scrollbar-hide pb-4 px-2 snap-x snap-mandatory">
                @foreach($organizations as $org)
                <a href="{{ route('panitia.show', $org->slug) }}"
                   class="group flex-shrink-0 w-[42%] sm:w-80 snap-start bg-white rounded-xl sm:rounded-3xl border border-slate-200 overflow-hidden hover:shadow-2xl hover:border-indigo-600 transition-all duration-300 cursor-pointer">
                    ...
                    <div class="relative h-20 sm:h-40 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 overflow-hidden">
                        ...
                        @else
                            ...
                            <div class="w-full h-full flex items-center justify-center">
                                <span class="text-2xl sm:text-7xl font-black text-white/30 group-hover:scale-110 transition duration-500">
                        ...
                        <!-- Badge Tipe di pojok -->
                        <span class="absolute top-1.5 right-1.5 sm:top-3 sm:right-3 inline-block px-1.5 py-0.5 sm:px-2 sm:py-1 bg-white/90 backdrop-blur text-indigo-600 text-[8px] sm:text-[10px] font-bold uppercase tracking-wider rounded-full">
                            {{ ucfirst($org->type) }}
                        </span>
                    </div>
                    ...
                    <div class="p-2 sm:p-5">
                        <h3 class="text-[11px] sm:text-lg font-black mb-0.5 sm:mb-1 line-clamp-1 group-hover:text-indigo-600 transition">
                            {{ $org->name }}
                        </h3>
                        <p class="hidden sm:block text-xs text-slate-500 line-clamp-2">
    ...
    <footer class="bg-indigo-900 text-indigo-100 py-20 px-6 mt-20">
        ...
        <div class="max-w-7xl mx-auto pt-12 mt-12 border-t border-indigo-800 text-center text-indigo-400 text-sm">
            &copy; {{ date('Y') }} AmikomEventHub. Seluruh hak cipta dilindungi.
        </div>
    </footer>
@endsection